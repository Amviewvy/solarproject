...
async function fetchLatestMeasurement(meterId: number) {
  ...
  return {
    id: latest.meter?.id ?? meterId,
     name: meterNames[meterId] ?? latest.meter?.name ?? `Meter ${meterId}`,
          ...
          <tbody>
            {data.map((row) => (
              <tr key={`${row.id}-${row.name}-${Math.random()}`}>
                <td className="w-12  align-middle px-0 pr-2">{row.id}</td>
                <td className="text-left align-middle pl-2 pr-12 font-medium">{row.name}</td>
                <td>{row.volts_avg}</td>
